...
export class ConsultoresService {
  ...
  listConsultores(): Observable<Consultor> {
    return this.http.get<Consultor>(this.API).pipe(tap(console.log));
  }
  getServicos(e): Observable<Servicos> {
    let _url: string = `http://localhost:8081/api/servicos-consultor/${encodeURI(
      e
    )}`;
    return this.http.get<Servicos>(_url).pipe(tap(console.log));
  }
  getFeriados(): Observable<Feriados[]> {
    let _url = "https://brasilapi.com.br/api/feriados/v1/2021";
    return this.http.get<Feriados[]>(_url).pipe(tap(console.log));
  }
